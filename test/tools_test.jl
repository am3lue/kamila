...
module MockKamilaEnv
    ...
    module KamilaMemory
        ...
        function track_activity(useful)
            return true
        end
        function add_achievement(title, desc)
            return true
        end
    end
    ...
    module TaskManager
        ...
        function add_task(title; kwargs...)
            return MockTask(1, title, get(kwargs, :priority, 2), get(kwargs, :due_date, nothing))
        end

        function get_pending_tasks()
            return [MockTask(1, "Test Task", 1, nothing), MockTask(2, "Review code changes", 2, nothing)]
        end
...
@testset "Kamila Tool Parts Tests" begin
    ...
    @testset "Task Tools" begin
        ...
        # complete_task error
        @test contains(execute_tool("complete_task", Dict("task_id" => 999)), "Error")
    end
