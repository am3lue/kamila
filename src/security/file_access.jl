...
function validate_path(path::String)
    ...
    if !is_path_allowed(expanded_path)
        throw(
            Errors.KamilaError(
                :permission,
                "Path '$path' is outside of allowed directories. Kamila can only access: $(join(ALLOWED_DIRS, ", "))",
...
function test_security()
    ...
    try
        ...
        if is_path_allowed(desktop_path)
            println("✅ Allowed directory access (Desktop): PASSED")
        else
            println("❌ Allowed directory access (Desktop): FAILED")
        end
    catch
    ...
    try
        ...
        if is_path_allowed(restricted_path)
            println("❌ Restricted path protection (/etc/passwd): FAILED")
        else
            println("✅ Restricted path protection (/etc/passwd): PASSED")
        end
    catch
        println("✅ Restricted path protection: PASSED")
    end

    # Test 3: Try to access a file within an allowed directory
    try
        test_file = joinpath(homedir(), "Desktop", "kamila_test.txt")
        if is_path_allowed(test_file)
            println("✅ File in allowed directory access: PASSED")
        else
            println("❌ File in allowed directory access: FAILED")
        end
    catch
        println("❌ File in allowed directory access: ERROR")
    end
end
